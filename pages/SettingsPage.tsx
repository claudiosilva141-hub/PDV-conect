...
export const SettingsPage: React.FC = () => {
  ...
  const handleSubmitCompanySettings = async (event: React.FormEvent) => {
    ...
    try {
      await updateCompanyInfo(companyName.trim(), logoBase64);
      setIsSaved(true);
      setTimeout(() => setIsSaved(false), 3000); // Hide saved message after 3 seconds
    } catch (err) {
      setError('Falha ao atualizar informações da empresa.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveUser = async (user: User) => {
    try {
      if (user.id && users.some(u => u.id === user.id)) { // Existing user (extra check to be sure)
        ...
        await registerUser(user.username, user.password || '', user.role);
      }
    } catch (error) {
      alert('Erro ao processar usuário.');
      throw error; // Re-throw for Modal to handle loading state if it caught it
    }
  };
