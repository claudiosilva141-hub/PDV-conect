...
export const LoginPage: React.FC = () => {
  ...
  const handleInitialSetupSubmit = async (event: React.FormEvent) => {
    ...
    // Backend will check if username exists and return error if it does
    // Update company info via context (calls API)
    updateCompanyInfo(currentCompanyName.trim(), logoBase64);
    registerUser(username.trim(), password.trim(), UserRole.ADMIN); // Register initial admin user (calls API)
  ...
  const handleLogin = async (event: React.FormEvent) => { // Made async
    event.preventDefault();
    setIsLoading(true);
    setError(null);
    ...
    const loginSuccess = await login(username.trim(), password.trim()); // Await the login call
    if (loginSuccess) {
      navigate('/', { replace: true });
    } else {
